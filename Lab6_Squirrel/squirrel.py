...
# Промена на 'WINWIDTH' за барање 1
...
# Нови константи за барање 2
HORIZONTAL_CAMERASLACK = 180  # how far from the center the squirrel moves horizontally before moving the camera
VERTICAL_CAMERASLACK = 90  # how far from the center the squirrel moves vertically before moving the camera

# Промена на 'CAMERASLACK' за барање 1
# CAMERASLACK = 90  # how far from the center the squirrel moves before moving the camera
CAMERASLACK = 180  # how far from the center the squirrel moves before moving the camera
...
# Додаваме нова константа за барање 4
LOSTSIZE = 10  # Size at which the game is considered lost

# Нова променлива која го чува бројот на поени - барање 5
playerScore = 0
...
def runGame():
    ...
    while True:  # main game loop
        ...
        while len(squirrelObjs) < NUMSQUIRRELS:
            squirrelObjs.append(makeNewSquirrel(camerax, cameray, bounceDirection='up'))
        ...
        '''
        # Промени за барање 2
        if (camerax + HALF_WINWIDTH) - playerCenterx > HORIZONTAL_CAMERASLACK:
            camerax = playerCenterx + HORIZONTAL_CAMERASLACK - HALF_WINWIDTH
        elif playerCenterx - (camerax + HALF_WINWIDTH) > HORIZONTAL_CAMERASLACK:
            camerax = playerCenterx - HORIZONTAL_CAMERASLACK - HALF_WINWIDTH
        if (cameray + HALF_WINHEIGHT) - playerCentery > VERTICAL_CAMERASLACK:
            cameray = playerCentery + VERTICAL_CAMERASLACK - HALF_WINHEIGHT
        elif playerCentery - (cameray + HALF_WINHEIGHT) > VERTICAL_CAMERASLACK:
            cameray = playerCentery - VERTICAL_CAMERASLACK - HALF_WINHEIGHT
            ...
            for i in range(len(squirrelObjs) - 1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size'] ** 2:
                        ...
                        # Инкрементирање на поените - Барање 5
                        playerScore += 1
                        ...
                        # Барање 4
                        # Големината на верверичката се намалува кога ќе ја допре поголема верверичка
                        playerObj["size"] -= (
                                int((playerObj["size"] * playerObj["size"]) ** 0.2) + 1
                        )

                        # Барање 4
                        # Сега играта има уште еден услов за завршување на играта
        ...
        # Прикажување на освоените поени на играчот - Барање 5
...
# Update the makeNewSquirrel function to include a bounceDirection parameter - Барање 3
def makeNewSquirrel(camerax, cameray, bounceDirection):
    sq = {}
    ...
    # Adjust movey based on the bounceDirection parameter
