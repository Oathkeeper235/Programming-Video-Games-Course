...
TIME_LIMIT = 300  # 300 seconds (5 minutes)
...
custom_image = pygame.image.load('your_custom_image.png')
...
def main():
    ...
    while True:  # main game loop
        ...
        timeRemaining = max(0, TIME_LIMIT - int(timeElapsed))
...
def drawTimer(timeRemaining):
    # Create a font object for the timer display
    font = pygame.font.Font(None, 36)
    text = font.render(f'Time Left: {timeRemaining}', True, WHITE)
...
def gameWonAnimation(board):
    ...
    # Create a variable to control the animation effect
    image_visible = False

    for i in range(13):
        color1, color2 = color2, color1  # Swap colors

        # Display the board and the custom image
        DISPLAYSURF.fill(color1)
        drawBoard(board, coveredBoxes)
        ...
        if image_visible:
            ...
            image_rect = custom_image.get_rect()
            image_rect.center = (WINDOWWIDTH // 2, WINDOWHEIGHT // 2)
            DISPLAYSURF.blit(custom_image, image_rect)
            image_visible = False
        else:
            image_visible = True
...
def gameOverAnimation():
    # Display a game over message with a background
    font = pygame.font.Font(None, 36)
    ...
    backgroundRect = pygame.Rect(textRect.left - 10, textRect.top - 10, textRect.width + 20, textRect.height + 20)

    # Fill the background rectangle with a color
    pygame.draw.rect(DISPLAYSURF, NAVYBLUE, backgroundRect)

    # Display the text on top of the background
    DISPLAYSURF.blit(text, textRect)
    pygame.display.update()

    # Wait for a moment before exiting the program
    pygame.time.wait(2000)
    pygame.quit()
    sys.exit()
